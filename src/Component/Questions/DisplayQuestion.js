...
function DisplayQuestion() {
  ...
  return (
      ...
      <thead className='bg-gray-50 p-4'>
        <tr>
          ...
          <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
            <select value={type} onChange={(e) => setType(e.target.value)} className='w-full sm:w-40 px-6 py-3 rounded-md'>
      ...
      <tbody className='bg-white divide-y divide-gray-200'>
        {filteredQuestions.length > 0 ? (
          filteredQuestions.map((ques) => (
            <tr key={ques._id}>
              <td className='px-6 py-4  overflow-hidden w-1/2'>{ques.Question}</td>
              <td className='px-6 py-4 whitespace-nowrap'>{ques.Category || '-'}</td>
              <td className='px-6 flex py-4 whitespace-nowrap text-right justify-center'>
          ...
          <tr>
            <td colSpan='3' className='text-center py-4'>No questions as of now</td>
          </tr>
        )}
      </tbody>
    </table>
    ...
    {model ? (
      <form onSubmit={addQuestion} className='bg-black fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-10 w-full sm:w-2/3 lg:w-1/2 mx-auto rounded-lg shadow-2xl'>
        ...
        <div className='flex flex-col'>
          <label className='text-white my-2'>Options</label>
          <input
            value={Option}
